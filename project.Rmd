...
Ulitmately, a patient's gender, whether they visited Wuhan recently, and what 
...
was defined to be 40-59, we chose 60 to be our cut-off age. In this 
hypothesis test, our null hypothesis was that the proportion of deaths in
both the younger and older groups would be equal.
$$H_0: p_{old} = p_{young}$$
...
ggplot(data = null_dist_prop, mapping = aes(x = stat)) +
...
than that of people younger than the age of 60.
...
We will determine whether the variables of interest are good predicters of 
...
We then created training and testing datasets covid_train and covid_test. 
...
## II. Mean Age of Affected Individuals
mean age CI interval for those infected is closer to interval to those that 
survive isn't the case that just older people are infected, those that are 
infected usually survive but of those infected the ones who are older are the 
ones that die (?)
...
interval for all patients, whereas the confidence interval for the mean age of patients who died was approximately 15 years higher than the others. 

## III. Association between Age Group and Death Rate

Through utilizing simulation-based two-sided hypothesis tests, we further 
examined the impact of the patient’s gender and whether they lived in Wuhan on 
the death rates. In both hypothesis tests, we rejected the null hypotheses that 
there is not difference in death proportions between genders and those who did 
and did not live in Wuhan. Therefore, there was a significant difference in the 
proportion of deaths between females and males, as well as those who did and 
those who did not live in Wuhan.
...
Given the nature of disease spread, we cannot becertain that our observations 
...
If we were to start over on this project, we would pick a data frame with more observations outside of China. For example, when considering country of origin 
...
age could easily have been confounded with other variables such as heart disease 
or diabetes status. Next time, we would find a dataset containing this 
information. Finally, another method we could have used to determine the 
significance of each explanatory variable on death rate would be inference for regression. The next steps we would do to continue this project would be to 
...
research questions, surrounding whether death outcome is correlated with factors 
like age and gender. Moreover, given that there are very few data points outside
of Asian countries, we were unable to draw meaningful conclusions to our 
research questions outside of these countries; it would be interesting to find 
whether our conclusions would hold in the U.S. and Europe. Ideally, we would 
want to create a way to update our data so that our analyses are done on real 
time data.
