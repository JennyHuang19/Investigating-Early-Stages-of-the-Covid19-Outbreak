...
for an outcome of "death" given the explanatory variables in our model. We will
determine whether these variables are good predicters of "death" through looking 
at the accuracy of our logistic regression model.

A new dataset called covid_class is created, selecting for our variables of 
interest (death, age, gender, visiting_wuhan, from_wuhan) and removing all "NA"
values.
...
A patient that is female, age 0, and has neither visited or is from Wuhan is
...
we would expect the log-odds of a covid case resulting in death to increase by approximately 2.37039736.
...
we saw that holding all other variables constant, for each unit increase in age, 
we would expect the log-odds of a covid case resulting in death to increase 
...
of the classifier (90%) we created from our model, we determined that the 
variables of interest are good predictors of death.

Classification using logistic regression is appropriate here, as our response
variable (death) is categorical and our predictors include both categorical
and continuous variables. The conditions to perform logistic regression are
that the observations are independent of one another and that the predictors 
are not highly correlated. Given the nature of disease spread, we cannot be 
certain that our observations are independent from one another, because one case 
could have caused another. Furthermore, there may be some correlation between 
the predictors "from_wuhan" and "visited_wuhan", because people who visit wuhan 
are likely to be from regions near Wuhan. These above-mentioned factors may 
lower the reliability of our coefficient estimates.

Finally, there were several missing values present among each of the 
variables of interest. Given that we filtered out these "NA" values before 
creating the logistic model, we have to consider that the incompleteness of the 
data we are working with makes our model is less valid and representative of the 
true situation.
