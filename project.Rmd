...
c) To get more information than sipmly seeing whether the confidence 
...
COVID-19.
## II. Confidence Intervals for Each Age Group
...
## III. Mean Age of Affected Individuals
...
both the younger and older groups would be equal. However, unlike our initial
attempt in our data analysis section, here we will create a null distribution
from the difference in mean death rates between our two groups, rather than
a distribution of mean death rates for only the "young" group. Though
computationally very similar, we felt this approach was more intuitive to read
from a plot.
$$H_0: p_{old} = p_{young}$$
...
$$H_A: p_{old} > p_{young}$$
```{r age hypothesis test, fig.width= 5, fig.height=4}
set.seed(123)
covid_age <- covid %>%
