...
determiners of death is by creating logistic regression models for 
classification for death. 

Pick 100 random observations to set aside as our testing data. 

```{r create-indices}
set.seed(03052020)
indices <- sample(nrow(covid), 50) # 100 observations to test on
```

Create training and testing datasets as well as vectors containing the status
of death for training and testing datasets.
...
Note: There is missing data for the variables: age-group, gender, and visiting
Wuhan; these missing data points may affect the accuracy of our logistic 
regression model.
